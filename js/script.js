...
function renderTimeline(data) {
  ...
  data.forEach((item, index) => {
    ...
    if (isLastTwo) {
          ...
          <div class="special-card-content">
            <div class="special-card-text">
              <div class="timeline-card-time-info">
                ${item.startTime && item.endTime ? `<p class="timeline-time timeline-time-range">${item.startTime} - ${item.endTime}</p>` : ""}
                ${item.timeSpent ? `<span class="timeline-time-spent">${item.timeSpent}</span>` : ""}
              </div>
      ...
      timelineContent = `
        ...
        <div class="timeline-card skye-card">

            <div class="timeline-card-time-info">
            ${item.startTime && item.endTime ? `
              <p class="timeline-time">
                <svg class="timeline-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  // Add event listeners to directions buttons to open modal
  document.querySelectorAll(".directions-button").forEach((button) => {
    button.addEventListener("click", (event) => {
      ...
      if (timelineCard) {
        const detailsElement = timelineCard.querySelector(".timeline-details");
        if (detailsElement) {
          const location = detailsElement.textContent;
...
function startCountdown() {
  ...
  let hours = 3, minutes = 2, seconds = 5;
  setupFlipdigitcards();
  ...
  function setupFlipdigitcards() {
    ...
    ['hours', 'minutes', 'seconds'].forEach((unit, index) => {
      ...
      if (unit === 'seconds') {
        group.classList.add('seconds-group');
      }

      group.id = unit;  // Keep this, it's correct.

      for (let i = 0; i < 2; i++) {
        const flipdigitcard = document.createElement('div');
        flipdigitcard.className = 'flip-digitcard';
        flipdigitcard.innerHTML = `
          <div class="flip-digitcard-inner">
            <div class="digitcard-top"><span>0</span></div>
            <div class="digitcard-bottom"><span>0</span></div>
          </div>`;
        group.appendChild(flipdigitcard);
      }

      countdownTimer.appendChild(group);

      if (unit !== 'seconds') {
        const separator = document.createElement('div');
        separator.className = 'separator';
        separator.textContent = unit === 'hours' ? 'h' : 'm';

        countdownTimer.appendChild(separator);
      }
    });
}
  ...
  function updateDigitGroup(id, value) {
    const formattedValue = value.toString().padStart(2, '0');
    document.querySelectorAll(`#${id} .flip-digitcard`).forEach((flipdigitcard, i) => {
      const digit = formattedValue[i];
      const flipTop = flipdigitcard.querySelector('.digitcard-top');
      const flipBottom = flipdigitcard.querySelector('.digitcard-bottom');
      const topSpan = flipTop.querySelector('span');
      const bottomSpan = flipBottom.querySelector('span');
      const currentValue = topSpan.textContent;

      if (currentValue !== digit) {
        flipAnimation(flipdigitcard, flipTop, flipBottom, digit, topSpan, bottomSpan);
      }
    });
  }

  function flipAnimation(flipdigitcard, flipTop, flipBottom, newDigit, topSpan, bottomSpan) {
    flipdigitcard.classList.add('flip');

    setTimeout(() => {
      topSpan.textContent = newDigit; // Change top text when animation is done
    }, 350); // Halfway through the flip

    setTimeout(() => {
      bottomSpan.textContent = newDigit; // Change bottom text at the end
      flipdigitcard.classList.remove('flip');
    }, 700);
  }
}
...
let lastUpdate = 0
const minDelay = 500 // Min time between emotional expression changes (ms)
let currentSentiment = 'neutral' // Track current emotional state
  ...
  sad: [
    {
      leftEye: "assets/icons/expressions/Left_Eye_Neutral.svg",
      rightEye: "assets/icons/expressions/Right_Eye_Neutral.svg",
      mouth: "assets/icons/expressions/Mouth_Neutral.svg",
      isHappy: false,
    }
  ],
  confused: [
...
function analyzeSentiment(text) {
  ...
  const happyWords = ['happy', 'great', 'excellent', 'amazing', 'love', 'wonderful', 'thank', 'thanks', 'good', 'awesome', 'excited', 'joy', 'pleased', 'delighted', 'yes', 'perfect', 'fantastic'];
  const sadWords = ['sad', 'sorry', 'disappointed', 'unhappy', 'bad', 'terrible', 'awful', 'upset', 'regret', 'miss', 'lost', 'worried', 'problem', 'issue', 'trouble'];
  const surprisedWords = ['wow', 'omg', 'oh my god', 'unbelievable', 'incredible', 'surprised', 'shocking', 'unexpected', 'amazing', 'astonishing', 'really', 'seriously'];
  const confusedWords = ['confused', 'what', 'how', 'why', 'don\'t understand', 'unclear', 'not sure', 'help', 'question', 'confusing', 'difficult', 'complicated'];
  ...
  surprisedWords.forEach(word => {
    if (lowerText.includes(word)) surprisedCount++;
  });

  // Check for confused words
  confusedWords.forEach(word => {
    if (lowerText.includes(word)) confusedCount++;
  });

  // Check for question marks (indicates confusion/question)
  if (text.includes('?')) confusedCount += 2;

  // Check for exclamation marks (indicates excitement/surprise)
  if (text.includes('!')) {
    happyCount++;
    surprisedCount++;
  }
  ...
  const sentiments = [
    { type: 'happy', count: happyCount },
    { type: 'sad', count: sadCount },
    { type: 'surprised', count: surprisedCount },
    { type: 'confused', count: confusedCount }
  ];

  // Sort by count (highest first)
  sentiments.sort((a, b) => b.count - a.count);

  // If no strong sentiment is detected, return neutral
  if (sentiments[0].count === 0) {
    return 'neutral';
  }

  return sentiments[0].type;
}
...
function animateEmoji(timestamp = 0) {
  ...
  if (timestamp - lastUpdate > minDelay) {
    // Get expressions based on current sentiment
    const sentimentExpressions = expressions[currentSentiment] || expressions.neutral;
    const randomIndex = Math.floor(Math.random() * sentimentExpressions.length);
    const expression = sentimentExpressions[randomIndex];

    changeExpression(expression)
    changeImmersiveExpression(expression)

    const emojiFace = document.querySelector(".emoji-face")
    if (emojiFace && expression.isHappy) {
...
function changeImmersiveExpression(expression) {
  if (immersiveLeftEye) immersiveLeftEye.src = expression.leftEye
  if (immersiveRightEye) immersiveRightEye.src = expression.rightEye
  if (immersiveMouth) immersiveMouth.src = expression.mouth
...
function startIdleAnimation() {
  ...
  function showRandomExpression() {
    // Use neutral expressions for idle animation
    const neutralExpressions = expressions.neutral;
    const randomIndex = Math.floor(Math.random() * neutralExpressions.length);
    const expression = neutralExpressions[randomIndex];
...
function changeExpression(expression) {
  if (!leftEyeImg || !rightEyeImg || !mouthImg) return;
...
function sendChatMessage() {
  ...
  currentSentiment = analyzeSentiment(message);
  console.log("Detected sentiment:", currentSentiment);
  ...
  }, 4400) // Increased delay to show more of the animation - Doherty threshold vs. visual experience?
}
...
function updateTimeline() {
  ...
  // We'll just insert it after the 2nd item (index 1) for most journeys
